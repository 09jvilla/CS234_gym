...
class Imitation():
    ...
    def run_expert(self, env, render=False):
        ...
        return Imitation.generate_expert_episode(env, render)
    ...
    def generate_expert_episode(env, render=False):
        # Generates an episode by running the given model on the given env.
        # Returns:
        # - a list of states, indexed by time step
        # - a list of actions, indexed by time step
...
def main(args):
    ...
    plot_model(agent.model, to_file='plots/model.png', show_shapes=True)
    ...
    for num_episodes in dataset_size:
        ...
        test_episode_size = 50
        for _ in range(test_episode_size):
            states, actions, rewards = agent.run_model(env, render)
            total_reward = np.sum(rewards)
            episode_rewards_model.append(total_reward)
        print('Traning size : ' + str(num_episodes) + ' Mean of rewards over ' + str(test_episode_size) + ' trials: ' + str(np.mean(episode_rewards_model)))
        print('Traning size : ' + str(num_episodes) + ' SD of rewards over '+  str(test_episode_size) +  ' trials: ' + str( np.std(episode_rewards_model)))
        print('\n')
